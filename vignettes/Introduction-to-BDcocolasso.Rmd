...
```{r}
plotCoef(fit_missing.lasso)
plotCoef(fit_missing.SCAD)
```

It is also possible to visualize the mean-squared error for all values of lambda:
```{r}
plotError(fit_missing.lasso)
plotError(fit_missing.SCAD)
```
...
fit_additive = coco(Z=Z_additive,y=y_additive,n=n_additive,p=p_additive,center.Z = FALSE, step=100,K=4,mu=10,tau=0.3,etol = 1e-4,noise = "additive", block=FALSE, penalty="lasso")
...
fit_additive = coco(Z=Z_additive,y=y_additive,n=n_additive,p=p_additive,p1=p1,p2=p2,center.Z = FALSE, step=100,K=4,mu=10,tau=0.3,noise="additive",block=TRUE, penalty="lasso")
