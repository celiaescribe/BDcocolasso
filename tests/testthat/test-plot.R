...
fit_missing = BDcocolasso::coco(Z=Z_missing,y=y_missing,n=n_missing,p=p_missing,step=100,K=4,mu=10,
                                tau=NULL, etol = 1e-4,noise = "missing", block=FALSE, penalty = 'lasso')

test_that("Plot returns ggplot object",{
  p1 <- BDcocolasso::plotCoef(fit_missing)
  expect_is(p1,"ggplot")
  
  p2 <- BDcocolasso::plotError(fit_missing)
  expect_is(p2,"ggplot")
})
